...
obj.cas_costfunctionvector = casadi.MX.zeros(m, 1);
...
for numCostFunction = 1 : m
    ...
    obj.cas_costfunctionvector(numCostFunction) = ...
    obj.costfunctionvector{numCostFunction}(obj.cas_variables);
end

% Use casadi to compute the jacobian of the cost function vector
obj.cas_costfunctionvectorjacobian = casadi.MX.zeros(m, n);
% Allocate the stacked casadi cost function hessians
obj.cas_costfunctionvectorstackedhessians = casadi.MX.zeros(n, n*m);
% For each cost function
for numCostFunction = 1 : m
    % Use casadi to compute the hessians and the gradients of the
    % individual cost functions from the vector
    [H, g] = hessian(obj.cas_costfunctionvector(numCostFunction), obj.cas_variables);
    % Store the gradient as a row in the cost function vector jacobian
    obj.cas_costfunctionvectorjacobian(numCostFunction, :) = g.';
    % Store the hessian as a submatrix among the stacked cost function 
    % vector hessians
    obj.cas_costfunctionvectorstackedhessians(:, (numCostFunction-1) * n + 1 : numCostFunction*n) = H;
end
